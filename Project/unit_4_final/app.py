...
def view_post(post_id):
    ...
    if post:
        return render_template('post.html', post=post[0], comments=comments, is_liked=is_liked, is_followed=is_followed)
...
def create_post():
    ...
    if request.method == 'POST':
        ...
        if category == 'Restaurants':
            ...
            create_post = f"INSERT INTO posts (title, category, content, datetime, image_name, user_id, rating, tags) VALUES ('{title}', '{category}', '{content}', '{datetime.now()}', '{filename}','{user_id}', '{rating}', '{tags}')"
            db.run_save(query=create_post)
        else:
            create_post = f"INSERT INTO posts (title, category, content, datetime, image_name, user_id, rating, tags) VALUES ('{title}', '{category}', '{content}', '{datetime.now()}', '{filename}','{user_id}', 0, 'None')"
            db.run_save(query=create_post)
...
def recipes():
    db = database_worker("shelfshare.db")
    posts = db.search("SELECT posts.*, users.name FROM posts INNER JOIN users ON posts.user_id = users.id WHERE posts.category = 'Recipe' ORDER BY posts.datetime DESC")
...
def locations():
    db = database_worker("shelfshare.db")
    posts = db.search("SELECT posts.*, users.name FROM posts INNER JOIN users ON posts.user_id = users.id WHERE posts.category = 'Location' ORDER BY posts.datetime DESC")
...
def restaurants():
    db = database_worker("shelfshare.db")
    posts = db.search("SELECT posts.*, users.name FROM posts INNER JOIN users ON posts.user_id = users.id WHERE posts.category = 'Restaurants' ORDER BY posts.datetime DESC")
